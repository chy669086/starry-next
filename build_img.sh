...
display_help()
{
	...
	echo "  -file | --testcase  If not specified, use the default testcase for different architectures."
	echo "  -s | --size		size of the disk image in 4MB batch size, default is set to 30, which means 120MB disk image"
	echo "  default testcase:"
...
if [ -z "$FILE" ]; then # use default testcase
...
# 如果 testcase 下对应测例不存在，执行 submodules 拉取
if [ ! -d "./testcase/$FILE" ]; then
...
if [ "x$os" = "xDarwin" ];then
	...
	# 根据命令行参数生成对应的测例
	echo "Copying $arch $fs $FILE/* to disk"
	sudo cp -r ./testcase/$FILE/* ./mnt/
	sudo umount mnt
	sudo rm -rf mnt
	sudo chmod 777 disk.img
fi

sudo mv disk.img .arceos/disk.img